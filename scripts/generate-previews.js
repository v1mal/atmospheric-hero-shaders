...
const PLAYGROUND_DIR = path.join(ROOT, "atmospheric-hero-shaders");
...
const PORT = 4173;
const BASE_URL = `http://${HOST}:${PORT}/atmospheric-hero-shaders`;
...
async function captureShader(browser, shader) {
  ...
  try {
    ...
    console.log(`Generated atmospheric-hero-shaders/previews/${shader.slug}.webp`);
  } finally {
    await page.close();
  }
}
...
async function main() {
  ...
  if (args.shader) {
    ...
    if (shaders.length === 0) {
      throw new Error(`Shader "${args.shader}" not found in atmospheric-hero-shaders/shaders.json`);
    }
  }
  ...
  try {
    ...
    try {
      for (const shader of shaders) {
        await captureShader(browser, shader);
      }
    } finally {
      await browser.close();
    }
